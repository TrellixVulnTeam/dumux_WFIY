...
#include <test/freeflow/navierstokesnc/compositionalanalyticalsolutionvectors.hh>
#include <test/freeflow/navierstokes/errors.hh>

#include "properties.hh"

template<class MassProblem>
auto createMassSource(const MassProblem& massProblem)
{
    using Scalar = double;

    const auto& gridGeometry = massProblem.gridGeometry();
    ...
    for (const auto& element : elements(gridGeometry.gridView()))
    {
        ...
        auto sourceAtPosVal = massProblem.sourceAtPos(center);
        source[0][eIdx] = sourceAtPosVal[0];
        source[1][eIdx] = sourceAtPosVal[1];
    }
...
auto createMomentumSource(const MomentumProblem& momentumProblem)
{
    using Scalar = double;
...
int main(int argc, char** argv)
{
    ...
    using MomentumTypeTag = Properties::TTag::AnalyticalNCTestMomentum;
    using MassTypeTag = Properties::TTag::AnalyticalNCTestMass;
    ...
    NavierStokesTest::CompositionalAnalyticalSolutionVectors analyticalSolVectors(momentumProblem, massProblem);
    ...
    const auto sourceMomentum = createMomentumSource(*momentumProblem);
    const auto& sourceMomentumX = sourceMomentum[MomentumProblem::Indices::momentumXBalanceIdx];
    const auto& sourceMomentumY = sourceMomentum[MomentumProblem::Indices::momentumYBalanceIdx];
    const auto sourceMass = createMassSource(*massProblem);
    const auto& sourceMassC = sourceMass[1];
    vtkWriter.addField(sourceMomentumX, "sourceMomentumX");
    vtkWriter.addField(sourceMomentumY, "sourceMomentumY");
    vtkWriter.addField(sourceMassC, "sourceMassC");
    vtkWriter.write(0.0);
